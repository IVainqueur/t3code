...
export function createThreadEnvironmentAtoms<R, E>(
  ...
  const commands = {
    ...
    dismissTask: createEnvironmentCommand(runtime, {
      label: "environment-data:commands:thread:dismiss-task",
      execute: (input: DismissSubagentTaskInput) => dismissSubagentTask(input),
      scheduler,
      concurrency,
    }),
    restoreTask: createEnvironmentCommand(runtime, {
      label: "environment-data:commands:thread:restore-task",
      execute: (input: RestoreSubagentTaskInput) => restoreSubagentTask(input),
      scheduler,
      concurrency,
    }),
