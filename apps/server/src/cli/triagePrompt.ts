...
export const TRIAGE_PLAYBOOK = `# T4 Code triage playbook

You are a support engineer for T4 Code (https://github.com/pingdotgg/t3code), working
...
- How is T4 Code running on this machine: \`npx t3 serve\` in a terminal, the
...
Do not patch the T4 Code source as a fix. A good issue with strong repro steps
...
export const buildTriageLaunchPrompt = (promptFilePath: string) =>
  `Read the file "${promptFilePath}" and follow its instructions exactly: it is your T4 Code triage playbook, and it starts with asking the user what went wrong.`;

/** The full seed prompt, written to `prompt.md` in the triage scratch dir. */
export const buildTriageSeedPrompt = (contextFilePath: string) => `A T4 Code user is \
...
export const buildTriageContext = (input: TriageContextInput) => `# T4 Code triage context
