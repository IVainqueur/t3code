...
function formatCloudStatus(status: CloudCliStatus, options?: { readonly json?: boolean }): string {
  ...
  const nextStep = !status.authenticated
    ...
    : !status.desired
      ...
      : !status.linked
        ? "Start T4 Code to provision the environment link and launch its managed tunnel."
        : undefined;
    ...
    runCloudCommand(
      ...
      Effect.gen(function* () {
        ...
        if (linked) {
          ...
          yield* Console.log(
            flags.publishOnly
              ? `✓ Authorized${connectedAs(linked.identity)}\n\nNext\n  Start T4 Code to publish agent activity (no managed tunnel).`
      ...
      Effect.gen(function* () {
        ...
        if (!enabled) {
          ...
          if (!linkedNow && (yield* CliState.readCliDesiredLinkMode) === "publish_only") {
            yield* CliState.setCliDesiredCloudLink(false);
            yield* Console.log("Cancelled the pending publish-only T4 Connect link.");
          }
        ...
        // out of band without T4 Connect.
        ...
        if (yield* CliState.readCliDesiredCloudLink) {
          yield* Console.log(
            "A T4 Connect link is already pending. Start T4 Code to finish provisioning it; publishing starts once it links.",
          );
          return;
        }
        yield* CliState.setCliDesiredCloudLink(true, "publish_only");
        yield* Console.log(
          "Restart T4 Code to finish authorizing this environment to publish (no managed tunnel is created).",
        );
      }),
    ),
  ),
...
export const connectCommand = Command.make("connect", {
  ...
  Command.withDescription("Set up T4 Connect for this machine."),
  Command.withHandler((flags) =>
    runCloudCommand(
      flags,
      Effect.gen(function* () {
