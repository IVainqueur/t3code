...
describe("orchestration projector", () => {
  ...
  effectIt.effect("folds task.dismissed then task.restored into dismissedTaskIds", () =>
    Effect.gen(function* () {
      const now = "2026-01-01T00:00:00.000Z";
      const eventFields = {
      ...
      expect(model.threads[0]?.dismissedTaskIds).toEqual([]);
      ...
      // Restoring must actually clear the id back to empty, not merely pass
