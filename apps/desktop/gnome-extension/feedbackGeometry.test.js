...
it("activates only a window of the process authenticated by D-Bus", () => {
  const target = window(42, "T4 Code");
  expect(findCaptureDestination([window(99, "T4 Code"), target], 42, "T4 Code")).toBe(target);
  expect(findCaptureDestination([window(99, "T4 Code")], 42, "T4 Code")).toBeUndefined();
