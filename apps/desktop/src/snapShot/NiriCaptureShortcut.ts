...
/** Niri owns the keybinding; this endpoint triggers capture without first focusing T4. */
export async function startNiriCaptureShortcut(
  ...
  try {
    ...
    if (result !== RequestNameReply.PRIMARY_OWNER)
      throw new Error("Another T4 Code instance already owns the capture shortcut.");
    return close;
  } catch (error) {
    close();
    throw error;
