...
export type SnapShotAnimationDestination = {
  ...
  /** Unit coordinates in T4's content area; GNOME supplies the real compositor origin. */
  ...
  private async showOverlays(
    ...
    try {
      ...
      if (this.waitForCompositorFrame) {
        // Cover the source with a composited snapshot before the caller reveals T4.
