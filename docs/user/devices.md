...
`agent-device` command line, which T4 Code sets up for them.
...
Studio's Device Manager. T4 Code detects standard SDK locations; set
...
timeline. Agents drive the device through the `agent-device` command line. T4
...
local network, Tailscale, and T4 Connect. Live video needs a secure page
...
T4 checks common Homebrew and Android SDK locations; custom installations need
...
sessions and stops its T4 helpers when reachable; simulators keep running.

T4 provides discovery, streaming, and control. Arrange app builds,
