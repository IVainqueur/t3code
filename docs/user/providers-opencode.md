...
T4 Code requires OpenCode 1.14.19 or newer, including when you connect an existing
OpenCode server.

## Local or external server

Leave **Server URL** empty to let T4 Code start OpenCode locally. A password in
provider settings applies to both that server and T4 Code's connection. With no
...
settings. T4 Code uses only that configured password for an external server; it
...
restart before T4 Code can see configuration changes.
