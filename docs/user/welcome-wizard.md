...
T4 Code shows a setup flow when you open a new installation or connect to the
hosted app for the first time. Existing workspaces skip this flow.

## Connect your computers

Select one or more computers to set up. If you opened T4 Code directly from a
...
- **T4 Connect** connects computers that are signed in to your account. Run
  `npx t3 connect` on each computer you want to add, then start T4 Code or run
...
Saved computers and computers discovered through T4 Connect are selected by
...
If T4 Code cannot confirm the workspace during startup, the setup flow shows
**Still connecting** instead of opening the app. Select **Reload** to try again.

If T4 Code cannot read your saved settings, it shows **Could not read settings**.
Select **Retry** after storage becomes available. Setup does not replace
unreadable settings with defaults.

## Check your agents

T4 Code checks each selected computer for Claude Code and Codex. If an agent is
...
T4 Code finds directories that Claude Code or Codex has used. Git repositories
...
A large or malformed history can reach the scan limit. T4 Code keeps the
...
30 days. You can continue those conversations in T4 Code.

Conversation import is best effort. T4 Code keeps the first user prompt and the
...
conversation so T4 Code does not remove user text. It reads one conversation at
