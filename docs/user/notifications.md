...
- **Finished a turn** — the agent stopped working and is waiting on you.
- **Needs approval** — a tool call is blocked until you allow or deny it.
- **Asked a question** — the agent is waiting on an answer before it continues.
- **Failed** — a turn ended in an error.

Each one has its own switch, so you can be told about blocked work without
being told about every completed turn.

## When they appear

A notification only appears when you are not already watching that thread. If
the thread is open in a focused window, nothing fires — you can already see
...
**Notification sound** plays your system's notification sound. Turn it off to
keep notifications silent while still seeing them.
