...
T4 Code runs Google's official Antigravity ACP agent on your selected environment.
...
**Sign in with Google** and complete the browser sign-in. Wait for T4 Code to confirm
...
hostname. Only that T4 Code sign-in session can finish the attempt. If it expires,
...
Use Antigravity's native `/plan` command for planning. T4 Code's separate Plan mode
...
T4 Code keeps conversation history and file diffs, but Antigravity cannot rewind
...
Put project skills in `.agents/skills`. T4 Code also reads `.gemini/skills` and the
