...
Device panel work over Tailscale and T4 Connect, including when an SSH host runs the devices.
...
session arbitrary command execution on the host, so T4 does not. The
...
snapshot model agents need and stays current with its own releases. T4 prepends
