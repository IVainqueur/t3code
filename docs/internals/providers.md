...
T4-managed OpenCode chat uses one server per thread. Its MCP registrations are directory-scoped, while
T4's MCP connection is thread-scoped. Sharing a chat server between threads in one directory would
...
T4 auth session. The client carries the return URL back to the environment because the provider's
