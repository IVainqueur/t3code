...
The screenshot harness runs the real mobile application against three disposable local T4
...
the software keyboard, and supplies deterministic T4 Connect discovery rows to the real
...
1. Creates three temporary T4 base directories and starts a local server for each on an available
   port.
2. Creates T4 Code, React, and Linux Git repositories with recognizable favicons, feature branches,
   and a deterministic T4 Code review diff.
...
The Pending rows use the production offline outbox and point at the real T4 Code and React fixture
