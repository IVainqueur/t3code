...
Labeling a PR `preview:mac` publishes a signed, notarized Apple Silicon DMG with T4 Connect enabled
...
- `.github/workflows/desktop-macos-preview-publish.yml` runs on `workflow_run` from `main`. It
  ...
  the PR. Only the version and the public T4 Connect identifiers in `.env.example` are read from the
